...
write.csv(gene_mapping, file.path(OUTPUT_DIR, "gene_id_to_symbol_mapping.csv"), row.names = FALSE)
...
# Save normalized counts with gene symbols
normalized_counts <- counts(dds, normalized = TRUE)
normalized_counts_df <- as.data.frame(normalized_counts)
normalized_counts_df$gene_id <- rownames(normalized_counts_df)
normalized_counts_df$gene_symbol <- gene_mapping[normalized_counts_df$gene_id, "gene_symbol"]
# Reorder columns to put gene_id and gene_symbol first
normalized_counts_df <- normalized_counts_df[, c("gene_id", "gene_symbol", sample_info$sample_name)]
write.csv(normalized_counts_df, file.path(OUTPUT_DIR, "normalized_counts.csv"), row.names = FALSE)
...
extract_results <- function(dds, contrast, output_dir) {
    ...
    res_df$gene_symbol <- gene_mapping[res_df$gene_id, "gene_symbol"]
    res_df <- res_df[, c("gene_id", "gene_symbol", "baseMean", "log2FoldChange", "lfcSE",
                          "stat", "pvalue", "padj")]
...
# Replace gene IDs with gene symbols for row labels
rownames(heatmap_data) <- gene_mapping[rownames(heatmap_data), "gene_symbol"]
