#!/usr/bin/env Rscript
# =============================================================================
# 14i_negonly_expanded.R
#
# Neg-only (exclusive) gene selection expanded to larger exon sizes.
# Unlike 14g which uses "neg_only + both" (all Neg-significant events),
# this script uses ONLY neg_only events (significant in Neg but NOT in Pos).
#
# ALL event types (SE, A3SS, A5SS, MXE, RI) are included, classified by
# computed exon_length to match the Venn diagram (14a) classification.
#
# Two groups:
#   Group 2b: Neg-only microexon + small exon (0-50bp) — all event types
#   Group 3b: Neg-only all sizes — all event types
#
# For context (Venn diagram numbers):
#   Microexon neg_only: 96, Small neg_only: 87, Regular neg_only: 729
#   Group 2b     = neg_only micro + small (all types) → expected ~183 events
#   Group 3b     = neg_only all sizes (all types)     → expected ~912 events
#
# Combined script: data preparation + 14f-style characterization.
#
# Output: results/14_todo_analysis/task5_expanded_groups/group{2b,3b}_*/
# =============================================================================
...
GROUPS <- list(
    group2b_negonly_micro_small = list(
        label = "Neg-Only Micro+Small (0-50bp)",
        description = "Genes with neg-only (not shared) microexon or small exon events (all types)",
        max_size = SMALL_MAX
    ),
    group3b_negonly_all_sizes = list(
        label = "Neg-Only All Sizes",
        description = "Genes with neg-only (not shared) events of any size (all types)",
        max_size = Inf
    )
)

# =============================================================================
# Utility Functions (same as 14a/14b/14g)
# =============================================================================
...
# =============================================================================
# PART 1: Load data and identify neg-only events
# =============================================================================
cat("========================================================\n")
cat("14i: Neg-Only Expanded Gene Selection + Characterization\n")
...
# Get neg-only events (significant in Neg, NOT significant in Pos) — ALL event types
neg_only <- neg_sig %>% filter(!(event_key %in% pos_sig$event_key))

cat(sprintf("\n  Neg significant events (all types): %d\n", nrow(neg_sig)))
cat(sprintf("  Pos significant events (all types): %d\n", nrow(pos_sig)))
cat(sprintf("  Neg-only events (not shared with Pos, all types): %d\n", nrow(neg_only)))
cat(sprintf("    By event type: SE=%d, A3SS=%d, A5SS=%d, MXE=%d, RI=%d\n",
            sum(neg_only$event_type == "SE"), sum(neg_only$event_type == "A3SS"),
            sum(neg_only$event_type == "A5SS"), sum(neg_only$event_type == "MXE"),
            sum(neg_only$event_type == "RI")))
cat(sprintf("    By size (all types):\n"))
cat(sprintf("      Microexon (0-30bp): %d\n", sum(neg_only$exon_length <= MICROEXON_MAX)))
cat(sprintf("      Small (30-50bp):    %d\n", sum(neg_only$exon_length > MICROEXON_MAX & neg_only$exon_length <= SMALL_MAX)))
cat(sprintf("      Regular (>50bp):    %d\n", sum(neg_only$exon_length > SMALL_MAX)))
...
for (group_name in names(GROUPS)) {
    ...
    # --- Select neg-only events within size cutoff (all event types) ---
    if (is.infinite(group$max_size)) {
        selection_events <- neg_only
    } else {
        selection_events <- neg_only %>% filter(exon_length <= group$max_size)
    }
    ...
    # --- Save selection data (14g-equivalent) ---
    ...
    # All Neg events for these genes
    ...
    # All Pos events for these genes
