#!/usr/bin/env Rscript
# =============================================================================
# 12_paper_comparison_double_kd.R
# =============================================================================
# Compare SRRM3 RNA-seq splicing data with Gonatopoulos-Pournatzis 2018
# DOUBLE KNOCKDOWN (Srrm3+Srrm4) data from VastDB (~54K events)
#
# This is the double-KD companion to 12_paper_comparison.R, which uses the
# 22-event curated single-KD dataset. The double-KD dataset is much larger
# and uses a different data format (standard rows=events layout vs transposed).
#
# Key differences from 12_paper_comparison.R:
#   - Input: data/srrm3_and_srrm4/ (54K VastDB events, standard layout)
#   - Coordinates: broad event regions (NOT exon boundaries) -> gene+size matching
#   - dPSI columns: Srrm3.Srrm4, Srrm3, and Srrm4 (3 conditions)
#   - Matching: gene name + exon size primary, containment disambiguation
#
...
base_output_dir <- file.path(base_dir, "results/12_paper_comparison_double_kd")
...
load_paper_data_double_kd <- function() {
  ...
  cat(sprintf("  - Srrm3+Srrm4 KD: %d events with dPSI\n", sum(!is.na(paper_data$srrm3_srrm4_dPSI))))
  cat(sprintf("  - Srrm3 KD: %d events with dPSI\n", sum(!is.na(paper_data$srrm3_dPSI))))
  cat(sprintf("  - Srrm4 KD: %d events with dPSI\n", sum(!is.na(paper_data$srrm4_dPSI))))
...
generate_plots <- function(comparison_df, stats, comparison, condition_label, output_dir) {
  ...
  for (cond_name in c("srrm3_srrm4", "srrm3", "srrm4")) {
    ...
    if (cond_stats$n >= 3 && !is.na(cond_stats$pearson$estimate)) {
      ...
      p <- ggplot(plot_data, aes(x = .data[[col_name]], y = rmats_dPSI)) +
        ...
        labs(
          title = sprintf("%s (Paper) vs %s (This Study)", cond_stats$label, condition_desc),
  ...
  if (length(valid_plots) >= 2) {
    ...
    do.call(grid.arrange, c(valid_plots, ncol = length(valid_plots),
                            top = sprintf("Double KD Comparison - %s", comparison)))
    dev.off()
  ...
  if (nrow(matched_for_heatmap) > 0) {
    ...
    p_heatmap <- ggplot(heatmap_data, aes(x = Condition, y = gene_label, fill = dPSI)) +
      ...
      labs(
        title = sprintf("dPSI Values - Top %d Events - %s", nrow(matched_for_heatmap), comparison),
        subtitle = "Comparing paper knockdowns with this study (ranked by |rMATS dPSI|)",
        x = "",
        y = "Gene (VastDB ID)"
      ) +
      theme_publication +
  ...
  p_summary <- ggplot(match_summary, aes(x = match_type, y = n, fill = match_type)) +
    ...
    labs(
      ...
      subtitle = sprintf("Total paper events: %d | Gene+size matching (±%dbp tolerance)",
                         nrow(match_df), size_tolerance),
      x = "Match Type",
      y = "Number of Events"
    ) +
    theme_publication +
...
save_outputs <- function(comparison_df, stats, multi_match_df, comparison, output_dir) {
  ...
  if (nrow(multi_match_df) > 0) {
    ...
    writeLines(sprintf("Multi-Match Event Report: %s (Double KD)", comparison), con)
...
cat("=============================================================================\n")
cat("SRRM3 Multi-Comparison Paper Validation - DOUBLE KNOCKDOWN\n")
cat("Gonatopoulos-Pournatzis et al. 2018 - Srrm3+Srrm4 Data\n")
cat("=============================================================================\n\n")
