#!/usr/bin/env Rscript
# =============================================================================
# 14h_se_expanded_pos_characterization.R
#
# SE-ONLY version of 14h_expanded_pos_characterization.R
#
# 14f-style deep characterization of significant Pos_vs_Parental events
# for each of the 3 expanded gene groups prepared by 14g_se.
#
...
#           summary_statistics
#
# Input:  results/14_todo_analysis/task5_expanded_groups_se/{group}/
# Output: results/14_todo_analysis/task5_expanded_groups_se/{group}/pos_significant_deep/
# =============================================================================
...
INPUT_BASE <- file.path(BASE_DIR, "results/14_todo_analysis/task5_expanded_groups_se")
...
# Group metadata
GROUP_LABELS <- list(
    group1_neg_microexon = "Microexon SE (0-30bp)",
    group2_neg_micro_small = "Microexon + Small SE (0-50bp)",
    group3_neg_all_sizes = "All SE Sizes"
)
...
# =============================================================================
cat("========================================================\n")
cat("14h_se: Expanded Pos Characterization (3 Groups, SE only)\n")
cat("========================================================\n")
...
for (group_name in groups) {
    ...
    neg_type_counts <- neg_sig %>% count(event_type, name = "count") %>% mutate(condition = "Neg_vs_Parental")
    pos_type_counts <- pos_sig %>% count(event_type, name = "count") %>% mutate(condition = "Pos_vs_Parental")
    ...
    neg_size_counts <- neg_sig %>% filter(event_type == "SE") %>% count(size_category, name = "count") %>% mutate(condition = "Neg_vs_Parental")
    pos_size_counts <- pos_sig %>% filter(event_type == "SE") %>% count(size_category, name = "count") %>% mutate(condition = "Pos_vs_Parental")
    ...
    neg_dir_counts <- neg_sig %>% count(direction, name = "count") %>% mutate(condition = "Neg_vs_Parental")
    pos_dir_counts <- pos_sig %>% count(direction, name = "count") %>% mutate(condition = "Pos_vs_Parental")
    ...
    if (nrow(genes_with_pos) > 0) {
        ...
        if (nrow(genes_with_pos) > max_heatmap_genes) {
            genes_with_pos <- genes_with_pos %>%
                ...
                arrange(desc(total)) %>% head(max_heatmap_genes)
            heatmap_subtitle <- sprintf("Top %d genes by total sig events (of %d with sig Pos)",
                                        max_heatmap_genes, sum(gene_profile$pos_n_sig > 0))
        } else {
            heatmap_subtitle <- sprintf("Genes with at least 1 significant Pos event (n = %d)",
                                        nrow(genes_with_pos))
        }

        heatmap_data <- bind_rows(
            neg_sig %>% filter(geneSymbol %in% genes_with_pos$geneSymbol) %>% mutate(condition = "Neg_vs_Parental"),
            pos_sig %>% filter(geneSymbol %in% genes_with_pos$geneSymbol) %>% mutate(condition = "Pos_vs_Parental")
        ) %>%
        ...
        p5 <- ggplot(heatmap_data, aes(x = event_type, y = geneSymbol, fill = count)) +
            ...
            labs(title = paste0("Significant Events per Gene: Neg vs Pos\n(", group_label, ")"),
                 subtitle = heatmap_subtitle, x = "Event type", y = "") +
    ...
    # Save CSVs
    write.csv(pos_sig, file.path(OUTPUT_DIR, "pos_significant_events_detail.csv"), row.names = FALSE)
    ...
    summary_stats <- data.frame(
        ...
        value = c(group_name, group_label, total_genes,
                  genes_with_sig_pos, genes_without_sig_pos,
                  round(100 * genes_with_sig_pos / total_genes, 1),
                  nrow(pos_sig),
                  sum(pos_sig$event_type == "SE"), sum(pos_sig$event_type == "A3SS"),
                  sum(pos_sig$event_type == "A5SS"), sum(pos_sig$event_type == "MXE"),
                  ...
                  sum(pos_sig$dPSI > 0), sum(pos_sig$dPSI < 0),
                  nrow(neg_sig),
                  ...
                  sum(neg_sig$dPSI > 0), sum(neg_sig$dPSI < 0)),
        stringsAsFactors = FALSE
    )
    ...
    for (et in names(event_colors)) {
        cat(sprintf("    %-5s: %3d / %3d\n", et,
                    sum(neg_sig$event_type == et), sum(pos_sig$event_type == et)))
    }
    cat("  Size breakdown - SE events only (Neg / Pos):\n")
    for (sc in names(size_colors)) {
        cat(sprintf("    %-20s: %3d / %3d\n", sc,
                    sum(neg_sig$event_type == "SE" & neg_sig$size_category == sc),
                    sum(pos_sig$event_type == "SE" & pos_sig$size_category == sc)))
    }
    cat("  Direction (Neg / Pos):\n")
    cat(sprintf("    Inclusion:  %3d / %3d\n", sum(neg_sig$dPSI > 0), sum(pos_sig$dPSI > 0)))
    cat(sprintf("    Exclusion:  %3d / %3d\n", sum(neg_sig$dPSI < 0), sum(pos_sig$dPSI < 0)))

    top_pos_genes <- gene_profile %>% filter(pos_n_sig > 0) %>% arrange(desc(pos_n_sig)) %>% head(10)
...
cat("\n\n========================================================\n")
cat("  CROSS-GROUP COMPARISON (SE only)\n")
cat("========================================================\n\n")
